...
%%
program: vars calculation SEMICOLON     {FILE *file = fopen(FILENAME, "w");
...
complex_op: IF expression THEN operator ELSE operator {$$ = create_ast_node_op('f'); add_child($$, $2); add_child($$, $4); add_child($$, $6);}
    |       compose_op                  {$$ = $1;};
